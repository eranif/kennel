...
// ---------------------------------------------------------------------------
// AuiFlatTabArt  –  iTerm2-Minimal-inspired tab art for macOS
// ---------------------------------------------------------------------------
//
// Visual design:
  ...
  void InitColours() {
    ...
    m_fgNormal = dark ? wxSystemSettings::GetColour(wxSYS_COLOUR_GRAYTEXT)
                      : wxSystemSettings::GetColour(wxSYS_COLOUR_GRAYTEXT)
                            .ChangeLightness(97);
    m_fgHilite =
        dark ? *wxBLACK : wxSystemSettings::GetColour(wxSYS_COLOUR_HOTLIGHT);
...
AuiFlatTabArt::AuiFlatTabArt() : wxAuiTabArtBase(), m_data(new Data()) {
  UpdateColoursFromSystem();
  InitBitmaps();
}

AuiFlatTabArt::AuiFlatTabArt(AuiFlatTabArt *other)
    : wxAuiTabArtBase(*other), m_data(new Data(*other->m_data)) {}

AuiFlatTabArt::~AuiFlatTabArt() { delete m_data; }

void AuiFlatTabArt::UpdateColoursFromSystem() { m_data->InitColours(); }

void AuiFlatTabArt::SetColour(const wxColour &colour) {
  m_data->m_fgNormal = colour;
}

void AuiFlatTabArt::SetActiveColour(const wxColour &colour) {
  m_data->m_fgActive = colour;
}

wxColour AuiFlatTabArt::GetButtonColour(wxAuiButtonId WXUNUSED(button),
...
wxAuiTabArt *AuiFlatTabArt::Clone() { return new AuiFlatTabArt(this); }
...
static int GetThinRectY(const wxRect &rect, bool bottom_tabs) {
  int y = bottom_tabs ? rect.GetBottom() - kThickness + 1
                      : rect.GetY() + kThickness - 1;
  return y;
}

static int GetThinRectTo(const wxRect &rect, bool bottom_tabs) {
  return bottom_tabs ? rect.GetY() : rect.GetBottom();
}

static wxColour GetDarkBorderColour(const wxColour &bgColour) {
  wxColour border_colour =
      IsDarkColour(bgColour)
          ? *wxBLACK
          : wxSystemSettings::GetColour(wxSYS_COLOUR_3DDKSHADOW);
  return border_colour;
}

void AuiFlatTabArt::DoDrawBackground(wxDC &dc, wxWindow *wnd,
                                     const wxRect &rect, bool with_bg) {
...
void AuiFlatTabArt::DrawBackground(wxDC &dc, wxWindow *wnd,
                                   const wxRect &rect) {
  DoDrawBackground(dc, wnd, rect, true);
}

void AuiFlatTabArt::DrawBorder(wxDC &dc, wxWindow *WXUNUSED(wnd),
                               const wxRect &rect) {
...
int AuiFlatTabArt::DrawPageTab(wxDC &dc, wxWindow *wnd, wxAuiNotebookPage &page,
                               const wxRect &rect) {
  ...
  const bool dark = IsDarkColour(m_data->m_bgWindow);
  ...
  if (page.active) {
    bool is_dark = IsDarkColour(m_data->m_bgWindow);
    // --- Fill the tab ---
    dc.SetBrush(bg);
    ...
    dc.DrawRoundedRectangle(tab_rect,
                            static_cast<double>(tab_rect.GetHeight()) / 2.0);
  }
  ...
  if (buttonsWidth) {
    ...
    all_buttons_rect.SetWidth(all_buttons_rect.GetWidth() + Data::PADDING_X +
                              Data::MARGIN);
  ...
  const wxString &text =
      wxControl::Ellipsize(page.caption, dc, wxELLIPSIZE_END, xEnd - xStart);
  const wxSize text_size = dc.GetTextExtent(text);
  const int textHeight = text_size.y;
  const int textWidth = text_size.x;
  wxRect text_rect{xStart, rect.y + (size.y - textHeight - 1) / 2, textWidth,
                   textHeight};
  if (buttonsWidth == 0) {
...
wxSize AuiFlatTabArt::GetPageTabSize(wxReadOnlyDC &dc, wxWindow *wnd,
...
int AuiFlatTabArt::GetBestTabCtrlSize(wxWindow *wnd,
...
int AuiFlatTabArt::GetIndentSize() { return 0; }

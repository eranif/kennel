...
wxColour GetBorderColour() {
  if (!kBorderShadow.IsOk()) {
    if (wxSystemSettings::GetAppearance().IsDark()) {
      kBorderShadow =
          wxSystemSettings::GetColour(wxSYS_COLOUR_3DFACE).ChangeLightness(120);
    } else {
...
// This private struct contains colours etc used by AuiFlatTabArt. Having it
...
struct AuiFlatTabArt::Data {
  ...
  void InitColours() {
    ...
    bool is_dark = IsDarkColour(m_bgActive);
...
void AuiFlatTabArt::DoDrawBackground([[maybe_unused]] wxDC &dc,
...
void AuiFlatTabArt::DrawBackground(wxDC &dc, wxWindow *WXUNUSED(wnd),
                                   const wxRect &rect) {
...
void AuiFlatTabArt::DrawBorder(wxDC &dc, wxWindow *wnd, const wxRect &rect) {
#if defined(__WXMSW__)
  wxDCBrushChanger brush_changer{dc, *wxTRANSPARENT_BRUSH};
  wxDCPenChanger pen_changer{dc,
...
int AuiFlatTabArt::DrawPageTab(wxDC &dc, wxWindow *wnd, wxAuiNotebookPage &page,
  ...
  auto activeTheme = ThemeManager::Get().ActiveTheme();
  if (activeTheme) {
    activeTabTextColour = activeTheme->fg;
    if (page.active)
      bg = activeTheme->bg;
  }
  ...
  dc.DrawLine(page.rect.GetLeft(), page.rect.GetTop(), page.rect.GetLeft(),
              page.rect.GetBottom());
  dc.DrawLine(page.rect.GetLeft(), page.rect.GetTop(), page.rect.GetRight(),
              page.rect.GetTop());
  ...
  if (page.active) {
    ...
    const int y = m_flags & wxAUI_NB_BOTTOM ? page.rect.GetBottom() - THICKNESS
                                            : page.rect.GetTop();

    dc.DrawRectangle(page.rect.GetLeft() + 1, y, page.rect.GetWidth() - 1,
                     THICKNESS);
    dc.SetPen(GetBorderColour());
